...
{
    flash()
    ll d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,x,y,a,b,r1,r2,r3,r4,r8x,lftx,dwny,upy,u;
    cin>>t;
    while(t--)
    {
        cin>>n;
        lftx=upy=-100000,r8x=dwny=100000;
        while(n--)
        {
            cin>>x>>y>>l>>u>>r>>d;
            if(l==0) lftx=max(x,lftx);
            if(u==0) dwny=min(y,dwny);
            if(r==0) r8x=min(x,r8x);
            if(d==0) upy=max(y,upy);

        }
       // cout<<r8x<<" "<<lftx<<" "<<upy<<" "<<dwny;nl; //r8x>lftx
        if(lftx>r8x || upy>dwny)  cout<<0<<endl;
        else    cout<<1<<" "<<(lftx+r8x)/2<<" "<<(dwny+upy)/2<<endl;
