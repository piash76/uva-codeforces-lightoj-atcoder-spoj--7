...
ll solve(ll pos,ll moves,ll cnt)
{
  if(pos>=n) return 0;
  if(dp[pos][moves][cnt]!=-1) return dp[pos][moves][cnt];

  ll ans=0;
  ans=max(ans,solve(pos+1,moves,cnt+(s[pos]==t[0]))+cnt*(s[pos]==t[1]));///no moves

  if(moves)
  {
      ans=max(ans,solve(pos+1,moves-1,cnt+1)+cnt* (t[0]==t[1]) );             ///making s[pos]==t[0]

      ans=max(ans,solve(pos+1,moves-1,cnt+(t[0]==t[1]))+cnt) ; /// ///making s[pos]==t[1]
  }

  return dp[pos][moves][cnt]=ans;
}
