...
int main()
{
    ...
    ll n,k;
    cin>>n>>k;
    vector<ll> al,bo,albo;
    ll x=0,y=0;
    f2(i,1,n)
    {
        ll t,a,b;
        cin>>t>>a>>b;
        if(a==1 and b==1) { albo.pb(t);x++;y++; }
        else if(a==1 and b==0) { al.pb(t);x++; }
        else if(a==0 and b==1) { bo.pb(t);y++; }

    }

    sort(all(albo));sort(all(al));sort(all(bo));

    if(x<k or y<k) cout<<-1<<endl;
    else
    {
        vector<ll> ans;
        for(auto x:albo) ans.pb(x);
        ll sz=min(al.size(),bo.size());
        for(i=0;i<sz;i++) ans.pb(al[i]+bo[i]);
        sort(all(ans));
        ll sum=0;
        f1(i,0,k) sum+=ans[i];
        c(sum);
