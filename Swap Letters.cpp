...
int main()
{
    ...
    ll i,j,t,cas=0;
        ...
        if(cnt1%2 or cnt2%2) cout<<-1<<endl;
        else
        {
            ...
            f1(i,0,n)
            {
                ...
                if(s1[i]=='a' and s2[i]=='b') ab.pb(i+1);
                else if(s1[i]=='b' and s2[i]=='a') ba.pb(i+1);
            }

            if(ab.size()%2)   ///a b ||| b a
            {
                ans.pb({ba.back(),ba.back()});
                ans.pb({ab.back(),ba.back()});
                ab.pop_back();ba.pop_back();

            }

            ll sz=ab.size();

            for(i=0;i<sz;i+=2) ans.pb({ab[i],ab[i+1]});

            sz=ba.size();

            for(i=0;i<sz;i+=2) ans.pb({ba[i],ba[i+1]});
